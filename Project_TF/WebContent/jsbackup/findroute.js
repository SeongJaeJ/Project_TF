		...
		success:function(data){
			...
			if(transportType == 1){//다음
				...
				for(var z=0; z<jsonObject.in_local.routes.length; z++){
					...
					$('#routesNav').append('<button onclick="searchRouteByDaum('+z+')">경로'+(z+1)+
				...
				if(transportType == 3){ //걷기의 경우 변수명이 약간 다름
					...
					$('#routesNav').append('총거리:'+distan.toFixed(2)	+'km<br>소요시간:'+timewaste+'분<br>이동속도:'+jsonObject.result.summary.speed+'km');
					...
					for(var z=0; z < jsonObject.routes.length; z++){
						...
						$('#routesNav').append('<button onclick="searchRouteByNaver('+z+')">경로'+(z+1)+'</button> 총거리:'+distan.toFixed(2)
