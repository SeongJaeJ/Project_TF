			...
			naver.maps.Event.addListener(map, 'rightclick', function(e){
				...
				var tttmp = new daum.maps.LatLng(latYlngX.y, latYlngX.x);
				console.log("tttmp:"+tttmp);
				console.log(tttmp.toCoords().jb, tttmp.toCoords().ib);
			...
			function button1_start(){
				if(startMark == null){
					startMark = new naver.maps.Marker({
						...
						animation: 2,
						icon: {
							url: '${contextPath}/img/startpoint.png',
			...
			function button2_via(){
				if(markerIndex > 5){
					...
					var viaMark = new naver.maps.Marker({
						...
						animation: 2,
						icon: {
							url: '${contextPath}/img/via'+markerIndex+'.png',
			...
			function button3_end(){
				if(endMark == null){
					endMark = new naver.maps.Marker({
						title: '도착지',
						icon: {
							url: '${contextPath}/img/endpoint.png',
							anchor: new naver.maps.Point(0, 42)
						},
						animation: 2,
						position: latYlngX,
...
// 			});
			...
			function transDaumCoordList(location){
				for(var i=0; i<location.length; i++){
					...
					daumCoordList.push(daumCoord.toCoords().jb+","+daumCoord.toCoords().ib);
				}
...
// 						console.log(jsonObject.in_local.routes[routesIndex].steps[j].type);
						...
						if(whatTypeTransPort == 'BUS'){
							whatUseTransport = '${contextPath}/img/bus.png';
						}else if(whatTypeTransPort == 'SUBWAY'){
							whatUseTransport = '${contextPath}/img/subway.png';
						}
